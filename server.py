...
FRONTEND_ORIGINS = {x.strip().rstrip("/") for x in FRONTEND_ORIGIN.split(",") if x.strip()}
...
# If FRONTEND_ORIGIN is empty, same-origin access still works, but every
# cross-origin call from the Netlify frontend will be blocked by the browser.
allowed_origins = sorted(FRONTEND_ORIGINS)
if PRODUCTION and not allowed_origins:
    print(
        "WARNING: FRONTEND_ORIGIN is not set. Every request from your Netlify site "
        "to this Render backend will fail with a CORS/'Failed to fetch' error until "
        "you set FRONTEND_ORIGIN in Render's Environment Variables to your exact "
        "Netlify URL (e.g. https://your-site.netlify.app — https://, no trailing "
        "slash, no path). You can pass a comma-separated list to allow more than "
        "one origin (e.g. your Netlify domain and a custom domain)."
    )
...
def require_active_ai_access(request: Request):
    # This edition is permanently free and the frontend no longer has a sign-in
    # step, so AI access does not require a session. If a signed-in session
    # cookie happens to be present (e.g. an older client), it is used to
    # personalize the profile context; otherwise the request proceeds as a
    # guest. Abuse is limited by the per-IP rate_check() calls at each AI route.
...
# IMPORTANT: CORSMiddleware is registered AFTER security_headers on purpose.
# Starlette wraps the LAST-registered middleware on the OUTSIDE of the stack,
# so registering CORS here (rather than above security_headers) makes it the
# outermost layer. That guarantees every response this server sends — including
# the 403 that security_headers returns for a cross-site request, and any error
# response from a route — carries the CORS headers the browser needs to read it.
# With the previous ordering, security_headers ran first and could return its
# 403 before CORSMiddleware ever touched the response; the browser would then
# have no Access-Control-Allow-Origin header to check and would report the
# whole request as a generic "Failed to fetch" instead of a readable 403.
app.add_middleware(
    CORSMiddleware,
...
def log_event(body: dict, request: Request):
    # The frontend fires this after every analysis and does not require
    # sign-in, so this endpoint must not require it either (it logs guest
    # activity with a null user_id when there is no session).
    user = get_user(request)
    ...
    with db() as conn:
        conn.execute(
            ...
            (user["id"] if user else None, event, detail, int(time.time())),
        )
    return {"ok": True}
...
def profile_context(user) -> str:
    if not user:
        return (
            "No verified customer profile is available for this guest session. "
            "Do not assume the user's age, gender, income or any other personal "
            "detail beyond what they state directly in the conversation."
        )
    return (
...
def stocks_ai_research(body: StockResearchRequest, request: Request):
    ...
    return {
        "symbol": symbol,
        "source": f"{OPENROUTER_MODEL if OPENROUTER_API_KEY else GEMINI_MODEL} (educational knowledge; no live quote feed)",
...
def stocks_ai_chat(body: AIRequest, request: Request):
    ...
    for message in body.messages[-12:]:
        role = "model" if message.role == "assistant" else "user"
        contents.append({"role": role, "parts": [{"text": message.content[:5000]}]})
    messages = [{"role": "user", "content": c["parts"][0]["text"]} for c in contents]
    return {"reply": generate_ai(STOCKS_AI_INSTRUCTIONS, messages)}
